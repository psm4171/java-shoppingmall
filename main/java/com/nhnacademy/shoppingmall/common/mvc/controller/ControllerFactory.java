...
//    public ControllerFactory(ConcurrentMap<String, Object> beanMap) {
//    }

    public void initialize(Set<Class<?>> c, ServletContext ctx){

        if(Objects.isNull(c)){
            log.info("Coontroller not fund");
            return;
        }
        ...
        for(Class<?> clazz : c) {
            try {
                ...
                if(requestMapping != null){
                    ...
                    for(String url : urls){
                        String key = requestMethod.name() + "-" + url;
        ...
        log.debug("controllerFactory - init");
